...
/*
pid_t do_over_http(int *f_in, int *f_out)
{
pid_t pid;
FILE *fp;
fp  = fopen("testhttp.txt","w");
pid = fork();
if(pid == 0)
{
fprintf(fp,"in the child of http");
fclose(fp);
}
...
if (curl){
    ...
    fprintf(fp, "=-=-=-=-=-=-=-=-=-=%s", buf);
   curl_easy_setopt(curl, CURLOPT_URL, "192.168.16.211:8080/a.html"); 
...
//	}
   curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "fuck");
